...
import type { AppModule as WalletsAppModule } from '@/app.module';
...
import type { WalletsController } from '@/presentation/controllers/wallets.controller';

describe('application bootstrap integration', () => {
	let app: INestApplication;
	let AppModule: typeof WalletsAppModule;
	let ControllerClass: typeof WalletsController;
	...
	beforeAll(async () => {
		...
		const [
			...
			{ WalletsController: ImportedControllerClass },
			...
			import('@/presentation/controllers/wallets.controller'),
		...
		globalThis.__walletsCreateSwaggerDocument = createSwaggerDocument;
		globalThis.__walletsCreateHttpExecutionContext = createHttpExecutionContext;
	...
	test('wires the authentication guard through Nest dependency injection', async () => {
		...
		await expect(
			guard.canActivate(
				globalThis.__walletsCreateHttpExecutionContext({
					headers: {},
				}),
			),
	...
	test('returns the authenticated player for the me controller action', async () => {
		const guard = app.get(GuardClass);
		const controller = app.get(ControllerClass) as WalletsController & {
		...
		await guard.canActivate(
			globalThis.__walletsCreateHttpExecutionContext(request),
		);
	...
	test('builds the OpenAPI document with bearer auth on the me route', () => {
		const document = globalThis.__walletsCreateSwaggerDocument(app);
